...
import com.example.demo.controller.ProjectBean;
import com.example.demo.controller.User;
import com.example.demo.xss.XssProperties;
import com.example.demo.xss.clean.DefaultXssClean;
...
public class FormConvertBeanResolver implements HandlerMethodArgumentResolver {
    ...
    public FormConvertBeanResolver(ConfigurableBeanFactory beanFactory) {
    ...
    public FormConvertBeanResolver() {
        this.annotationNotRequired = true;
        this.useDefaultResolution = false;
    ...
    public boolean supportsParameter(MethodParameter parameter) {
        ...
        if (parameter.hasParameterAnnotation(FormConvertBean.class)) {
            return true;
        } else {
    ...
    private Object bindRequestParameters(MethodParameter parameter, NativeWebRequest webRequest) throws Exception {
        ...
        FormConvertBean parameterAnnotation = parameter.getParameterAnnotation(FormConvertBean.class);
        //注解的value是JSON的key
    ...
    protected NamedValueInfo createNamedValueInfo(MethodParameter parameter) {
        FormConvertBean ann = parameter.getParameterAnnotation(FormConvertBean.class);
     ...
     * @param request the current request
     */
    protected void bindRequestParameters(WebDataBinder binder, NativeWebRequest request) throws InstantiationException, IllegalAccessException {
        ...
        PropertyEditor customEditor = binder.getBindingResult().getPropertyEditorRegistry().findCustomEditor(String.class, null);

        Arrays.stream(target.getClass().getDeclaredFields()).forEach(e->extracted(new String[]{""},e,servletRequest,customEditor,wrapper));
    }

    private static void extracted(String[] keys,Field field,HttpServletRequest servletRequest, PropertyEditor customEditor,BeanWrapper wrapper) {
        Class<?> wrappedClass = field.getType();
        String name = field.getName();
        String _name = name;
        JsonProperty jsonProperty = field.getAnnotation(JsonProperty.class);
        if (jsonProperty!=null){
            _name = jsonProperty.value();
        }

        if (BeanUtils.isSimpleProperty(wrappedClass)) {
            String[] values  = getParameterValues(keys,servletRequest,customEditor,_name,name);
            wrapper.setPropertyValue(name,values);
        }else{
            if (Map.class.isAssignableFrom(wrappedClass)) {
                // map

            }else if(Collection.class.isAssignableFrom(wrappedClass)){
                // Collection
                wrapper.setPropertyValue(name,getParameterValues2Collection(keys,servletRequest,customEditor,_name,name));

            }else {
                Object propertyValue = wrapper.getPropertyValue(name);
                if(propertyValue==null){
                    propertyValue = BeanUtils.instantiateClass(wrappedClass);
                }
                wrapper.setPropertyValue(name,propertyValue);
                BeanWrapper beanWrapper = PropertyAccessorFactory.forBeanPropertyAccess(propertyValue);
                String[] _keys = new String[]{_name+".",name+"."};
                Arrays.stream(wrappedClass.getDeclaredFields()).forEach(e->extracted(_keys,e,servletRequest,customEditor,beanWrapper));
            }
        }
    }

    private static String[] getParameterValues(String[] keys,HttpServletRequest request ,PropertyEditor editor, String ...args){
        for (String arg : args) {
            for (String key: keys) {
                String[] values = request.getParameterValues(key+arg);
                if(values!=null && values.length>0){
                    String[] _values = new String[values.length];
                    for (int i = 0; i < values.length; i++) {
                        editor.setAsText(values[i]);
                        _values[i] = editor.getAsText();
                    }
                    return _values;
                }
            }
    ...
    private static Collection<String>  getParameterValues2Collection(String[] keys,HttpServletRequest request ,PropertyEditor editor, String ...args){
        return Arrays.asList(getParameterValues(keys,request,editor,args));
    }
    ...
    public boolean supportsReturnType(MethodParameter returnType) {
        if (returnType.getMethodAnnotation(FormConvertBean.class) != null) {
            return true;
        } else
    ...
    private static class RequestCustomParamNamedValueInfo extends NamedValueInfo {
        ...
        public RequestCustomParamNamedValueInfo(FormConvertBean annotation) {
